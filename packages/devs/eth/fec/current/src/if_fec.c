...
mxc_fec_read_mac_from_fuse *get_mac_addr = NULL;
	...
	for(i = 0, p = priv->rx_cur; i< FEC_RX_FRAMES; i++){
		...
		*	when the CACHE_LINE write back.
		...
		if((p->status & BD_RX_ST_ERRS)|| (p->length > FEC_FRAME_LEN)) {
			//diag_printf("BUG1[RX]: status=%x, length=%x\n", p->status, p->length);
		} else {
...
mxc_fec_chip_init(mxc_fec_priv_t * dev)
{
	...
	while (chip->miigsk_enr & MIIGSK_ENR_READY)
		hal_delay_us(FEC_COMMON_TICK);

	/* configure gasket for RMII, 50 MHz, no loopback, and no echo */
	chip->miigsk_cfgr = MIIGSK_CFGR_IF_MODE_RMII;

	/* re-enable the gasket */
	chip->miigsk_enr = MIIGSK_ENR_EN;

#if 0 // Spec says READY should reflect EN but doesn't seem to here
	/* wait for the gasket to be ready */
	while ((chip->miigsk_enr & MIIGSK_ENR_READY) == 0)
		hal_delay_us(FEC_COMMON_TICK);
#endif
#endif
...
#ifdef MX51_ERDOS
/*
 * callback PHY found/notfound
 */
void (*fec_phy_callback)(int phy_found);
#endif /* MX51_ERDOS */
/*!
...
mxc_fec_phy_init(mxc_fec_priv_t * dev)
{
	...
	case 0x00540088:
		break;
	case 0x0007c0c0:
		diag_printf("FEC LAN8700 PHY: ID=%lx\n", id);
		break;
	default:
		diag_printf("[Warning] FEC not connect right PHY: ID=%lx\n", id);
#ifdef MX51_ERDOS
		if (fec_phy_callback != 0) {
			(*fec_phy_callback)(0);
		}
		return false;
#endif /* MX51_ERDOS */
	...
	for ( i = 0 ; i < 100 ; i++ ) {
		mxc_fec_mii_read(dev->hw_reg, dev->phy_addr, PHY_CTRL_REG, &value);
		if ((value & PHY_CTRL_RESET) == 0) {
			break;
		}
	}
	if (100 <= i) {
		diag_printf ("phy_init: PHY RESET failed CTRL_REG:%x %d\n", value, i);
	}
#endif /* MX51_ERDOS */
...
#ifdef MX51_ERDOS
	/*
	 * wait complete auto-nego
	 */
	for ( i = 0 ; i < 10000 ; i++ ) {
		mxc_fec_mii_read(dev->hw_reg, dev->phy_addr, PHY_STATUS_REG, &value);
        	if ( value & 0x0030 ) {
			/*
			 * autoneg complete or remote fault
			 */
			break;
		}
		hal_delay_us ( 1000 );
  	}
	if ((value & 0x0020) == 0x0000) {
		diag_printf ("phy_init: AutoNEGO failed STATUS_REG:%x %d\n", value, i);
	}
#endif /* MX51_ERDOS */
...
mxc_fec_init(struct cyg_netdevtab_entry *tab)
{
	...
	unsigned char eth_add_local[6] = {0x00, 0x00, 0x45, 0x67, 0x89, 0xab};
	cyg_bool set_esa;
	int ok;
#ifdef CYGSEM_REDBOOT_FLASH_CONFIG
	/* Get MAC address */
...
#endif
	/* See if a MAC address is specified in the fuse registers */
	if (!(ok && set_esa)) {
		if (get_mac_addr)
			ok = get_mac_addr(eth_add_local);
	}
	 ...
	 * check have PHY
	 */
	if (mxc_fec_phy_init(private) == false) {
		return false;
	}
#else
