...
// ------------------------
// 2009/07/27 : hal_diag_read_char_nowait() add.
//              cyg_hal_plf_serial_silent() add.
// 2009/08/06 : console show restart at cyg_hal_plf_serial_silent(1->0)
//
//===========================================================================*/
...
#ifdef MX51_ERDOS
#define SILENT_MSG_LEN  (2048)
static int  silent_flag;
static int  silent_msg_ofs;
static char silent_msg [SILENT_MSG_LEN + 16];
/*
 * serial silent mode
 */
void cyg_hal_plf_serial_silent (int mode)
{
    int oflag = silent_flag;
    silent_flag = mode;
    if ((mode == 0) && (oflag != 0)) {
        /*
         * re-enable serial console
         *  display saved charactor
         */
        int i, j, k;
        volatile struct mxc_serial *base = ((channel_data_t *)&(channels [0]))->base;
        CYGARC_HAL_SAVE_GP();
        i = silent_msg_ofs;
        for ( j = 0 ; j < SILENT_MSG_LEN ; j++ , i++ ) {
            if (SILENT_MSG_LEN <= i) {
                i = 0;
            }
            if (silent_msg [i] != '\0') {
                ...
                for ( k = 0 ; k < 0x10000 ; k++ ) {
                    if ((base->uts & EUartUTS_TXFULL) == 0) {
                        base->utxd [0] = silent_msg [i];
                        break;
                    }
                }
            }
...
#ifdef MX51_ERDOS
    if (silent_flag == 1) {
        if ((silent_msg_ofs < 0) || (SILENT_MSG_LEN <= silent_msg_ofs)) {
            silent_msg_ofs = 0;
        }
        silent_msg [silent_msg_ofs++] = c;
        return;
    }
#endif /* MX51_ERDOS */
...
#ifdef MX51_ERDOS
/*
 * no wait keyinput check
 *  rc 0 - no keyin
 *     1 - keyin valid [*c]
 */
int hal_diag_read_char_nowait (char *c)
{
    cyg_bool in;

    CYGARC_HAL_SAVE_GP();
    in = cyg_hal_plf_serial_getc_nonblock(&(channels [0]), (cyg_uint8 *)c);
