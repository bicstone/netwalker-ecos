...
// ------------------------
// 2009/07/10 : when sdstart() ERROR, error_loop() delete goto normal boot.
// 2009/07/21 : QA0 message support.
// 2009/07/27 : autostart timeout 0 try one keyinput.
// 2009/07/29 : stop_normal_boot() add.
// 2009/08/01 : stop_normal_boot() delete at sdstart().
// 2009/08/06 : stop_normal_boot() add at not entry script.
// 2009/08/12 : boot first script support.
//              board name(Spider -> PC-Z1) version add.
//
//==========================================================================
...
do_version(int argc, char *argv[])
{
    ...
    diag_printf("Platform: %s [%s] (%s) %s\n", "MX51 SHARP PC-Z1(QA0)", bootVersion, HAL_PLATFORM_CPU, HAL_PLATFORM_EXTRA);
#else
    diag_printf("Platform: %s [%s] (%s) %s\n", "MX51 SHARP PC-Z1", bootVersion, HAL_PLATFORM_CPU, HAL_PLATFORM_EXTRA);
...
#ifdef MX51_ERDOS
static char *getline (char *src, char *dst)
{
    char *p = src;
...
#ifdef MX51_ERDOS
    {
        extern unsigned int get_gpio_sw_init (void);
        extern int sdstart (int slot, char *conf, char *cmdline);
        extern int hal_diag_read_char_nowait (char *c);
        extern char *b1st_script;
        unsigned int sw = get_gpio_sw_init ();

        /*
         * boot first script
         */
        if (b1st_script != 0) {
            char *line = b1st_script;
            char  scmd [128];
            if ((hal_diag_read_char_nowait (scmd) == 1) && (scmd [0] == 0x03)) {
                /*
                 * keyin [^C]
                 */
                script_abort = 1;
                diag_printf("== Executing boot first script abort ==\n");
            } else {
                diag_printf("== Executing boot first script ==\n");
                while ((line = getline (line, scmd)) != 0) {
                    char *p = scmd;
                    if ((*p == '#') || (*p == '\n') || (*p == '\0')) {
                        ;		/* comment */
                    } else if (*p == '=') {
                        ...
                        diag_printf ("RedBoot> %s\n", p);
                        cmd = parse (&p, &argc, &argv [0]);
                        if (cmd != 0) {
        ...
        if (script_abort == 0) {
            /*
             * SW L/R check
             *    L:ON R:ON -> SD1 Kernel start
             */
            if ((sw & 0x0003) == 0x0003) {
                int rc = sdstart (1, (char *)0, (char *)0);
            }
        }
    }
#endif /* MX51_ERDOS */
    ...
    if (script) {
        ...
        if (script_abort == 0) {
            if (script_timeout_ms == 0) {
                extern int hal_diag_read_char_nowait (char *c);
                if ((hal_diag_read_char_nowait (line) == 1) && (line [0] == 0x03)) {
                    /*
                     * keyin [^C]
                     */
                    res = _GETS_CTRLC;
        ...
        if (res == _GETS_CTRLC) {
#ifdef MX51_ERDOS
            extern void stop_normal_boot (void);
            stop_normal_boot ();
#endif /* MX51_ERDOS */
