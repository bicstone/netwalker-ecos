...
/*
 * pre script
 *  run fast script before boot_script
 */
RedBoot_config_option("Run script boot first",
                      b1st_script,
                      ALWAYS_ENABLED, true,
                      CONFIG_BOOL,
                      false
    );
RedBoot_config_option("Boot first script",
                      b1st_script_data,
                      "b1st_script", true,
                      CONFIG_SCRIPT,
                      ""
...
#if defined(MXCFLASH_SELECT_NAND)
extern int mxc_nand_get_page_size(void);
        if (IS_FIS_FROM_NAND()) {
            cfg_size = _rup(sizeof(struct _config), mxc_nand_get_page_size());
        }
#endif
...
#ifdef MX51_ERDOS
    {
        bool use_b1st_script;
        flash_get_config ("b1st_script", &use_b1st_script, CONFIG_BOOL);
        if (use_b1st_script) {
            flash_get_config ("b1st_script_data", &b1st_script, CONFIG_SCRIPT);
        }
    }
#endif /* MX51_ERDOS */
