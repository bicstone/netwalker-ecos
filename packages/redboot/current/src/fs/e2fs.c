...
// ------------------------
// 2009/07/13 : dbg_e2fs_read() add.
// 2009/07/14 : inode_size support.
//
//==========================================================================
...
#ifdef MX51_ERDOS
static int dbg_e2fs_flag;
static e2fs_desc_t *e2fs_orig;
#endif /* MX51_ERDOS */
...
#ifdef MX51_ERDOS
    offset = ((ino - 1) % e2fs->inodes_per_group) * e2fs->inode_size;
#else
...
#ifdef MX51_ERDOS
    e2fs->inode_size = SWAB_LE16(sb->s_inode_size);
#endif /* MX51_ERDOS */
		 ...
		 const char *name, int namelen)
{
	...
	reclen = SWAB_LE16(dir->reclen);
        ...
        if ( dbg_e2fs_flag ) {
            char fname [64];
            memcpy (fname, dir->name, dir->namelen);
            fname [dir->namelen] = '\0';
            diag_printf("search_dir_block: %d/%d %d [%s](%d) [%s](%d)\n", offset, e2fs->blocksize, reclen, name, namelen, fname, dir->namelen);
        }
#endif /* MX51_ERDOS */
	offset += reclen;
	len = dir->namelen;
		...
		const char  *name, int         namelen)
{
...
#ifdef MX51_ERDOS
        if ( dbg_e2fs_flag ) {
            diag_printf ("e2fs_dir_lookup: [1] [%s] %d\n", name, dir_ino);
        }
#endif /* MX51_ERDOS */
	return NULL;
    }
	...
	if (!e2fs_inode_block(e2fs, &inode, i, &block_nr)) {
            if ( dbg_e2fs_flag ) {
                diag_printf ("e2fs_dir_lookup: [2] %d/%d\n", i, nblocks);
            }
	    return NULL;
        }
        if ( dbg_e2fs_flag ) {
            diag_printf ("e2fs_dir_lookup: [%s] %d/%d dir_ino:%d sect:%d %d %d\n", name, i+1, nblocks, dir_ino, E2FS_BLOCK_TO_SECTOR(e2fs, (block_nr)), nbytes, e2fs->blocksize);
        }
#else
	if (!e2fs_inode_block(e2fs, &inode, i, &block_nr))
	    return NULL;
	...
	if (block_nr) {
	    ...
	    if (!__READ_BLOCK(block_nr)) {
                if ( dbg_e2fs_flag ) {
                    diag_printf ("e2fs_dir_lookup: [3] %d\n", block_nr);
                }
		return NULL;
            }
#else
...
#ifdef MX51_ERDOS
    if ( dbg_e2fs_flag ) {
        diag_printf ("e2fs_dir_lookup: [4] %d\n", block_nr);
    }
#endif /* MX51_ERDOS */
    return NULL;
}
...
e2fs_inode_lookup(e2fs_desc_t *e2fs, cyg_uint32 dir_ino, const char *pathname, ino_info_t *info)
{
...
#ifdef MX51_ERDOS
    if ( dbg_e2fs_flag ) {
        diag_printf("e2fs_inode_lookup: [%s]\n", pathname);
    }
#endif /* MX51_ERDOS */
	...
	if (dir == NULL) {
            if ( dbg_e2fs_flag ) {
                diag_printf("e2fs_inode_lookup: [1] %d\n", len);
            }
	    return 0;
        }
#else
	...
	switch (dir->filetype) {
	  case E2FS_FTYPE_SYMLINK:
	    ...
	    if (!e2fs_follow_symlink(e2fs, dir_ino, SWAB_LE32(dir->inode), info)) {
                if ( dbg_e2fs_flag ) {
                    diag_printf("e2fs_inode_lookup: [2] %d %d\n", dir_ino, SWAB_LE32(dir->inode));
                }
		return 0;
            }
#else
	    ...
	    // must be a dir if we want to continue
            ...
            if (info->filetype != E2FS_FTYPE_DIR) {
                if ( dbg_e2fs_flag ) {
                    diag_printf("e2fs_inode_lookup: [3] %x\n", info->filetype);
                }
		return 0;
            }
#else
	  ...
	  case E2FS_FTYPE_REG_FILE:
	    ...
	    if (pathlen) {
                if ( dbg_e2fs_flag ) {
                    diag_printf("e2fs_inode_lookup: [4] %d\n", pathlen);
                }
...
#ifdef MX51_ERDOS
            if ( dbg_e2fs_flag ) {
                diag_printf("e2fs_inode_lookup: [5] %d\n", dir->filetype);
            }
#endif /* MX51_ERDOS */
	    return 0;
	}
...
#ifdef MX51_ERDOS
/*
 * dbg_e2fs - display flag
 */
static void dbg_e2fs (int argc, char *argv[])
{
    int mode;
    if (argc == 1) {
        diag_printf ("EXT2 debug mode %x\n", dbg_e2fs_flag);
        return;
    }
    if (!parse_num (*(&argv[1]), (unsigned long *)&mode, &argv[1], ":")) {
        diag_printf ("Error: Invalid mode parameter\n");
        return;
    }
    diag_printf ("EXT2 debug mode %x -> %x\n", dbg_e2fs_flag, mode);
    dbg_e2fs_flag = mode;
}
RedBoot_cmd("dbg_e2fs",
            "EXT2 debug mode",
            "<mode>",
            dbg_e2fs
           );
...
/*
 * dbg_e2fs_read - partition sector read
 */
static void dbg_e2fs_read (int argc, char *argv[])
{
    int           i, sector, usage;
    ...
    for ( i = 1 ; i < argc ; i++ ) {
        if (sector == -1) {
            if (!parse_num (*(&argv[i]), (unsigned long *)&sector, &argv[i], ":")) {
                usage++;
            }
        } else {
            usage++;
    ...
    if ((0 < usage) || (sector == -1)) {
        diag_printf ("Usage: dbg_e2fs_read <sector>\n");
        return;
    }
    if (!PARTITION_READ(e2fs_orig->part, sector, (cyg_uint32 *)buf, 1)) {
        diag_printf ("sector:%d read error\n", sector);
        return;
    }
    memset (mm, '.', 16);
    mm [16] = '\0';
